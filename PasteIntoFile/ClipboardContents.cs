...
public class ImageContent : ImageLikeContent {
    public static readonly string[] EXTENSIONS = { "png", "bmp", "gif", "jpg", "pdf", "tif", "ico", "webp" };
    ...
    public override void SaveAs(string path, string extension, bool append = false) {
        ...
        switch (NormalizeExtension(extension)) {
            ...
            case "webp":
                var bytes = Webp.Encode(image, true);
...
public class VectorImageContent : ImageLikeContent {
    ...
    public override PreviewHolder Preview(string extension) {
        switch (NormalizeExtension(extension)) {
            case "emf":
                var description = string.Format(Resources.str_preview_image_vector, Metafile.Width, Metafile.Height,
                    Math.Round(Metafile.HorizontalResolution / 2 + Metafile.VerticalResolution / 2));
...
public class TextLikeContent : BaseContent {
    ...
    protected void AddTo(IDataObject data, string text, Encoding encoding = null) {
        foreach (var f in Formats) {
            if (DataFormats.GetFormat(f).Id < 32) {
                // Native formats, handled well by default
                data.SetData(f, text);
            } else {
                // Non-native formats
...
public class CalendarContent : TextLikeContent {
    ...
    public Calendar Calendar => Calendar.Load(Text);
...
public class ClipboardContents {
    ...
    private static (string, byte[]) BytesFromDataUri(string uri) {
        try {
            ...
            /* data uri malformed or not supported */
        }
        return (null, null);
    }
